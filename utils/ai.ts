...
export const generateNutritionResponse = async (history: { role: 'user' | 'model', parts: { text: string }[] }[], newMessageText: string, imageBase64?: string): Promise<AIResponse> => {
    if (!apiKey) {
        return {
            isMeal: false,
            textResponse: 'שגיאת מערכת: מפתח API של Gemini חסר. אנא הוסף את EXPO_PUBLIC_GEMINI_API_KEY לקובץ .env שלך.'
        };
    }
        ...
        if (imageBase64) {
            parts.push({
                inlineData: {
                    ...
                    mimeType: 'image/jpeg' // Fallback, adjust if parsing dynamically
                }
            });
        }

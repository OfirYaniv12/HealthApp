import { Meal, getLogicalDayMeals } from '@/db/database';
import { useUserStore } from '@/store/useUserStore';
import { getLogicalDayBounds } from '@/utils/calculators';
import { Ionicons } from '@expo/vector-icons';
import { useFocusEffect, useRouter } from 'expo-router';
import React, { useCallback, useState } from 'react';
import { I18nManager, LayoutAnimation, Platform, SafeAreaView, ScrollView, StyleSheet, Text, TouchableOpacity, View } from 'react-native';
...
export default function NutritionalAnalysisScreen() {
    const router = useRouter();
    const user = useUserStore(state => state.user);
    const [meals, setMeals] = useState<Meal[]>([]);
    const [expandedTab, setExpandedTab] = useState<NutrientTab | null>(null);
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            <View style={styles.header}>
                ...
                <TouchableOpacity onPress={() => router.push('/customize-nutrients')} style={styles.settingsTextButton}>
                    <Text style={styles.settingsText}>התאמה אישית</Text>
                </TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
    ...
    settingsTextButton: { paddingHorizontal: 8, paddingVertical: 4 },
    settingsText: { color: '#3b82f6', fontSize: 16, fontWeight: '600' },
    container: { flex: 1 },
    ...
    progressBarFill: { height: '100%', backgroundColor: '#3b82f6', borderRadius: 4 }, // Consistent color theme
    progressPercentage: { fontSize: 13, fontWeight: 'bold', color: '#3b82f6', minWidth: 36, textAlign: 'center' },
